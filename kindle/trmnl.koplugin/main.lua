...
function TrmnlDisplay:displayImage(image_path)
    ...
    -- Long-press handler to close the image
    self.image_widget.onTapClose = function()
        logger.info("TRMNL: Closing image via long-press")
...
function TrmnlDisplay:onCloseWidget()
    ...
    -- njh patch: stopAutoRefresh() would persist auto_refresh_enabled=false, and this runs on
    -- every KOReader exit and FileManager <-> Reader switch, turning the dashboard off for good.
    -- Only stop this instance's timer; the next instance resumes from the saved setting.
    self:unscheduleRefreshTask()
    self.auto_refresh_scheduled = false
    self:allowAutoSuspend()
    if self.image_widget then
